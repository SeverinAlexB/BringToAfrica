...
public class DonationGoalServiceTest {
    ...
    @Test
    public void testGetStateInPercentWithSingleDonation() {
        DatabaseTest.runInCleanApp(testBrowser -> {
            Project project = getFilledDataStructure();
            int state1 = DonationGoalService.getStateInPercent(project.getDonationGoals().get(0));
            assertEquals(50, state1);
        });
    }

    @Test
    public void testGetStateInPercentWithMultipleDonations() {
        DatabaseTest.runInCleanApp(testBrowser -> {
            Project project = getFilledDataStructure();
            int state2 = DonationGoalService.getStateInPercent(project.getDonationGoals().get(1));
